...
- 파일·폴더 추가, 이름 수정, 삭제(실행취소 지원), `▲/▼` 고정 순서 변경
...
선택 정책·바로가기(.lnk) 대상 해석·트레이 큐·시작프로그램 명령·단일
인스턴스·실행취소·업데이트 확인·실제 Tk 토스트 생성 등을 검사합니다.
`.github/workflows/ci.yml`이 `main` 커밋과 모든 PR마다 Windows 러너에서 이
테스트 스위트를 자동 실행합니다.
