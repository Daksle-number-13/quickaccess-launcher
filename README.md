...
폴더, 파일, 웹 링크를 한 번의 클릭으로 여는 상주형 런처입니다.
...
환경 설정에서는 파일·폴더뿐 아니라 인터넷 링크도 추가할 수 있으며, 화면 스타일,
패널 열 수, 전역 단축키와 Windows 자동 실행을 한곳에서 변경할 수 있습니다.
...
- 사용자별 자동 실행 상태 확인·이전 EXE 경로 자동 복구, 한글 경로 및 UTF-8 JSON 지원
- 두 번 실행하면 새 프로세스를 남기지 않고 기존 앱의 패널 또는 설정창을 표시
- Windows 11 계열 카드 UI, 저장되는 밝기 모드, 혼합 DPI 모니터 대응
- 설정에서 새 버전 자동 확인을 켜면 GitHub 릴리스를 확인해 트레이 알림으로 안내
  (기본값은 꺼짐, 같은 버전은 한 번만 표시)
- 환경 설정을 `바로가기 / 단축키·시작 / 화면·패널 / 앱 정보`로 분리하고,
  설정 가져오기·내보내기와 개인정보를 제외한 진단 정보 복사 제공
- `앱 정보`에서 현재 앱 버전과 공개 제작자 이름(`Daksle`) 확인
- 설정 저장 위치: `%APPDATA%\QuickAccess\items.json`
  (원자 저장과 직전 백업, 제한된 손상 스냅샷으로 실수·파일 손상 복구)
...
`Ctrl+Shift+Space`를 누릅니다. 선택된 항목이 있으면 최대 50개를 선택 순서대로
한 번에 등록하고, 없으면 현재 폴더를 등록합니다. 중간 항목이 가상 위치처럼
안전하게 해석되지 않으면 일부만 저장하지 않고 전체 요청을 취소합니다. Quick
Access, 내 PC 등 실제 파일시스템 경로가 아닌 가상 위치는 등록하지 않습니다.
...
자동 테스트는 데이터 마이그레이션·원자 저장·손상 복구·설정 가져오기·고정 순서·
검색 순위·핫키 파싱과 롤백·작업 영역 경계 보정·경로 검사 타임아웃과 오래된 결과
무시·탐색기 COM 다중 선택 정책·바로가기(.lnk) 속성 보존·아이콘 캐시/중복 요청
방지·트레이 큐·시작프로그램 실제 상태와 경로 복구·단일 인스턴스 요청 전달·
실행취소·업데이트 확인·실제 Tk 토스트 생성 등을 검사합니다.
`.github/workflows/ci.yml`이 `main` 커밋과 모든 PR마다
...
[변경 기록](CHANGELOG.md) · [MIT License](LICENSE) ·
